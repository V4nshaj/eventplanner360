...
export const connectoDatabase = async () =>{
    if(cached.conn) return cached.conn;//cached.conn check if funcn already holds Mongodb(mongoose) connection This ensures that the function doesn't attempt to connect to MongoDB multiple times if a connection is already established and cached.
