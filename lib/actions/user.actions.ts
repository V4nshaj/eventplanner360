...
export async function createUser(user: CreateUserParams) {//accepts user data as parameter and tries to create a user in database. We need to define the type of data(i.e. user) we are passing to createUser
  try {
    ...
    return JSON.parse(JSON.stringify(newUser))//actual data we are passing to createUser we are doing this to get the general javascript object not necessarily MongoDB document
  } catch (error) {
    handleError(error)
  }
}
